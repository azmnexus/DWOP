...
export default function WorkforcePage() {
  ...
  const [error, setError] = useState<{
    type: "error" | "network" | "forbidden";
    message?: string;
  } | null>(null);
  const [addOpen, setAddOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<ProfessionalCreate>({
    first_name: "",
    last_name: "",
    email: "",
    phone: "",
  ...
  const fetchPeople = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getAllPages<ProfessionalRead>("/people/", {
        status_filter: statusFilter || undefined,
        availability_filter: availabilityFilter || undefined,
      });
      setPeople(data);
    } catch (err) {
  ...
  }, [availabilityFilter, statusFilter]);
        ...
        <h2 className={styles.directoryTitle}>
          ...
          {!loading && (
            <span className={styles.directoryCount}>
              {filteredPeople.length} professional
      ...
      <div className={styles.filtersBar}>
        ...
        <div className={styles.filterSelect}>
          <select
            ...
            value={statusFilter}
            onChange={(e) =>
              setStatusFilter(e.target.value as ProfessionalStatus | "")
            }
                ...
                >
                  <td>
                    <div className={styles.personCell}>
                      <div
                        className={uiStyles.avatar + " " + uiStyles.avatarMd}
                      >
                  ...
                  <td>
                    <Badge kind="professional-status" value={person.status} />
                  </td>
                  <td>
                    <Badge
                      label={
      ...
      >
        <form onSubmit={createProfessional}>
          <div className={uiStyles.formGrid}>
            ...
            <Input
              label="Skills"
              placeholder="FastAPI, React, Operations"
            ...
            />
            <div className={uiStyles.formRow}>
              <label className={uiStyles.inputWrapper}>
                <span className={uiStyles.inputLabel}>Status</span>
                <select
                  className={uiStyles.selectField}
                  value={form.status}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      status: e.target.value as ProfessionalStatus,
                    })
                  }
                >
                  <option value="intake">Intake</option>
              ...
              <label className={uiStyles.inputWrapper}>
                <span className={uiStyles.inputLabel}>Availability</span>
                <select
                  className={uiStyles.selectField}
