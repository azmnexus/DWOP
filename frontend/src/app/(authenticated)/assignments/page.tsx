...
export default function AssignmentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setClients(c);
      setPeople(professionals);
      setAverageCapacity(getCapacityUtilization(capacity));
    } catch (e) {
      setError(
  ...
  async function createProject(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim() || !form.code.trim() || !form.client_id)
      return showToast("warning", "Name, code, and client are required.");
    setSaving(true);
    try {
    ...
    <div className="fade-in">
      <div className={styles.capacityRow}>
        <div className={styles.statCard}>
          <BarChart3 size={22} />
          <div>
            <div className={styles.statValue}>{averageCapacity}%</div>
            <div className={styles.statLabel}>Average allocation</div>
            <div className={styles.capacityBar}>
              <div
                className={styles.capacityFill}
                style={{ width: `${Math.min(averageCapacity, 100)}%` }}
              />
            </div>
          </div>
        </div>
        <div className={styles.statCard}>
          ...
          <div>
            ...
            <div className={styles.statLabel}>Available professionals</div>
          </div>
        </div>
        <div className={styles.statCard}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h2 className={styles.pageTitle}>
            Projects{" "}
            <span className={styles.pageCount}>{filtered.length} projects</span>
          </h2>
          <p className={styles.pageSubtitle}>
            Assignments, capacity, dates, and project staffing in one
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Projects returned by the DWOP API will appear here."
        />
      ) : (
        <>
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr
                    ...
                    tabIndex={0}
                    role="link"
                    onClick={() => router.push(`/assignments/${p.id}`)}
                    onKeyDown={(e) =>
                      e.key === "Enter" && router.push(`/assignments/${p.id}`)
                    }
                  >
                    <td>
        ...
        <form onSubmit={createProject}>
          <div className={uiStyles.formGrid}>
            ...
            <div className={uiStyles.inputWrapper}>
              <label className={uiStyles.inputLabel}>Client</label>
              <select
                required
                className={uiStyles.selectField}
            ...
            <div className={uiStyles.inputWrapper}>
              <label className={uiStyles.inputLabel}>Status</label>
              <select
                className={uiStyles.selectField}
                value={form.status}
                onChange={(e) =>
            ...
            <div className={uiStyles.formRow}>
              ...
              <Input
                ...
                type="date"
                onChange={(e) =>
                  setForm({ ...form, target_end_date: e.target.value || null })
                }
              />
            </div>
          </div>
