...
export default function ProjectDetail() {
  ...
  const [project, setProject] = useState<ProjectRead | null>(null),
    [client, setClient] = useState<ClientRead | null>(null),
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [p, c, professionals] = await Promise.all([
        api.get<ProjectRead>(`/assignments/projects/${id}`),
        api.get<CapacityOverviewRead>("/assignments/capacity"),
        getAllPages<ProfessionalRead>("/people/"),
      ]);
      setProject(normalizeProject(p));
      setCapacity(getCapacityUtilization(c));
      setAvailable(c.available_headcount ?? 0);
      setPeople(professionals);
      if (p.client_id)
        setClient(await api.get<ClientRead>(`/clients/${p.client_id}`));
    } catch (e) {
      setError(
      ...
      <div className={styles.detailHeader}>
        <div>
          <div className={styles.detailTitleGroup}>
            ...
            <Badge kind="project-status" value={project.status} />
          </div>
          <p className={styles.pageSubtitle}>Project-backed delivery board</p>
        </div>
      </div>
      <div className={styles.detailGrid}>
        <Card>
          <h3 className={styles.sectionTitle}>Board overview</h3>
        ...
        <Card>
          ...
          <div className={styles.fieldRow}>
            <span className={styles.fieldLabel}>Target end</span>
            <span className={styles.fieldValue}>
        ...
        <Card>
          ...
          <div className={styles.capacitySummary}>
            <div
              ...
              style={{
                background: `conic-gradient(var(--color-brand-accent) ${Math.min(capacity, 100) * 3.6}deg,var(--color-surface-secondary) 0)`,
              }}
            >
              <span>{capacity}%</span>
            </div>
