...
export function WorkBoardPrototype({ people }: { people: ProfessionalRead[] }) {
  ...
  const memberPeople = useMemo(
    () => people.filter((p) => members.includes(p.id)),
    [people, members],
  );
  ...
  const move = (id: string, stage: Stage) =>
    setTickets(tickets.map((t) => (t.id === id ? { ...t, stage } : t)));
    ...
    <section className={styles.boardSection}>
      <div className={styles.boardToolbar}>
        <div>
          <div className={styles.prototypeTag}>
            Interactive prototype · session only
          </div>
          <h3>Delivery board</h3>
          <p>
            Model the workflow now; refresh clears placeholder members and
            tickets.
          </p>
        </div>
      ...
      <div className={styles.memberStrip}>
        ...
        {memberPeople.length ? (
          ...
          <small>No members added in this session</small>
        )}
      </div>
      <div className={styles.ticketBoard}>
        {stages.map((stage) => (
          <div className={styles.ticketLane} key={stage.id}>
            <header>
              <strong>{stage.label}</strong>
              <span>{tickets.filter((t) => t.stage === stage.id).length}</span>
            </header>
            <div className={styles.ticketStack}>
              {tickets
                .filter((t) => t.stage === stage.id)
                .map((t) => (
                  <article className={styles.ticketCard} key={t.id}>
                    <span
                      className={`${styles.priority} ${styles[t.priority]}`}
                    >
                      {t.priority}
                    </span>
                    <h4>{t.title}</h4>
                    ...
                    <label>
                      ...
                      <select
                        value={t.stage}
                        onChange={(e) => move(t.id, e.target.value as Stage)}
                      >
                        {stages.map((s) => (
              ...
              {!tickets.some((t) => t.stage === stage.id) && (
                <button
      ...
      >
        <form onSubmit={addMember}>
          <label className={ui.inputWrapper}>
            <span className={ui.inputLabel}>Professional</span>
            <select
              required
          ...
          <div className={ui.modalActions}>
            ...
            <Button
              type="submit"
              disabled={!selectedMember || members.includes(selectedMember)}
            >
              Add member
            </Button>
          </div>
      ...
      >
        <form onSubmit={createTicket}>
          <div className={ui.formGrid}>
            ...
            />
            <label className={ui.inputWrapper}>
              <span className={ui.inputLabel}>Description</span>
              <textarea
                className={ui.textareaField}
            ...
            <div className={ui.formRow}>
              <label className={ui.inputWrapper}>
                <span className={ui.inputLabel}>Priority</span>
                <select
                  className={ui.selectField}
            ...
            <label className={ui.inputWrapper}>
              <span className={ui.inputLabel}>Assignee</span>
              <select
                className={ui.selectField}
          ...
          <div className={ui.modalActions}>
            ...
            <Button type="submit">Create placeholder ticket</Button>
          </div>
        </form>
      </Modal>
    </section>
